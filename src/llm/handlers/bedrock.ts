...
import { BaseHandler } from "./base"; // Adjusted import
import { InputError, InvariantError, type MIMEType } from "./base"; // Adjusted import for types moved to base.ts
import {
  consoleWarn,
...
} from "./bedrock_utils"; // Adjusted import
...
const toChatCompletionChoiceMessage = (
  ...
  if (typeof toolChoice !== "string" && toolChoice?.type === "function") {
    ...
    const selectedTool = output.message.content
...
export const convertMessages = async (
  ...
  } else if (clonedMessages.length === 0) {
    clonedMessages.push({
      role: "user",
      content: "Empty",
    } as ChatCompletionMessageParam);
  ...
  for (const message of clonedMessages) {
    ...
    if (message.role === "tool") {
      ...
      if (Array.isArray(message.tool_calls)) {
        const convertedContent: ContentBlock.ToolUseMember[] =
      ...
      const text = makeTextContent(message.role as string, message.content);
      currentParams.push({
        text,
      });
      ...
      const convertedContent: (
        ...
        message.content.map(async (e) => {
          if (e.type === "text") {
            const text = makeTextContent(message.role as string, e.text);
            return {
              text,
            } as ContentBlock.TextMember;
          }
          if (e.type === "image_url" && e.image_url) {
          ...
          consoleWarn(
            `Unhandled content part type: ${e.type} in convertMessages`
          );
          return null;
        })
      );
      currentParams.push(
        ...convertedContent.filter(
          Boolean as unknown as (
            value: ContentBlock.ImageMember | ContentBlock.TextMember | null
          ) => value is ContentBlock.ImageMember | ContentBlock.TextMember
        )
      );
    }
...
const convertStopReason = (
  ...
  if (completionReason === "end_turn" || completionReason === "stop_sequence") {
    return "stop";
  }
  if (completionReason === "max_tokens") {
    return "length";
  }
  if (completionReason === "tool_use") {
    return "tool_calls";
  }
  return "unknown";
};
...
async function* createCompletionResponseStreaming(
  ...
  if (response.stream === undefined) {
    ...
    for await (const stream of response.stream) {
      ...
      if (stream.modelStreamErrorException) {
        throw stream.modelStreamErrorException;
      }
      if (stream.throttlingException) {
        throw stream.throttlingException;
      }
      if (stream.validationException) {
...
export class BedrockHandler extends BaseHandler<BedrockRuntimeClient> {
  validateInputs(
  ...
  async create(
    ...
    const client = new BedrockRuntimeClient({
      region,
      credentials: {
        // biome-ignore lint/style/noNonNullAssertion: they are checked in a way biome doesn't understand
